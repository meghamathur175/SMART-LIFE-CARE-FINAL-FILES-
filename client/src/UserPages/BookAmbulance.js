...
function BookAmbulance() {
  ...
  const [distanceDurationCost, setDistanceDurationCost] = useState({
    distance: "N/A",
    duration: "N/A",
    cost: "N/A",
  });
  ...
  const handleChange = (e) => {
    ...
    if (name === "vehicleType") {
      ...
      setAmbulanceCost(selectedOption?.price || 0);
  ...
  useEffect(() => {
    ...
    const fetchPartnerId = async () => {
      try {
        ...
        else console.warn("Partner ID not found");
      } catch (error) {
  ...
  const handleBookAmbulance = () => setShowBookingForm(true);
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      if (!partnerId) return alert("Invalid partner/hospital data");
      ...
      const response = await fetch(
        "http://localhost:3001/api/partners/confirm-request",
        {
          ...
          body: JSON.stringify({
            ...
            ambulanceCost:
              (ambulanceCost + Number(hospital.cost.replace("₹", "").trim())) *
              noOfAmbulances,
            distance: distance,
          }),
        }
      ...
      if (!response.ok) throw new Error(data.message || "Failed to confirm request");
      ...
      setTimeout(() => {
        ...
        navigate("/looking-for-driver", {
          state: {
            hospital: { ...hospital, _id: partnerId },
            data,
  ...
  const geocodeDestination = useCallback(() => {
    ...
    geocoder.geocode({ address: destinationLocation }, (results, status) => {
      if (status === "OK" && results[0]) {
        ...
        console.error("Failed to geocode destination location:", status);
  ...
  const handlePaymentMethod = () => setShowPaymentMethod(true);
    ...
    <div className="google-maps-layout">
        ...
        <div className="info">
          ...
          {hospital ? (
            <>
              ...
              <p className="info-item"><strong>Distance:</strong> {hospital.distance}</p>
              <p className="info-item"><strong>Duration:</strong> {hospital.duration}</p>
              <p className="info-item"><strong>Estimated Cost:</strong> {hospital.cost}</p>
              <p className="info-item"><strong>Rating:</strong> {hospital.rating || "Not Available"}⭐️</p>
            </>
          ) : (
          ...
          <div className="booking-ambulance-popup">
            <div className="booking-ambulance-popup-content">
              ...
              <div className="confirm-booking">
                <button className="confirm-ambulance-btn" onClick={handleConfirmBooking}>Confirm Booking</button>
                <button className="book-cancel-btn" onClick={() => setShowBookingForm(false)}>Cancel</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="book-map-container">
        <GoogleMap mapContainerStyle={containerStyle} zoom={13} center={centerPosition}>
          ...
          {pickupCoords && (
            <Marker position={pickupCoords} icon={{ url: pickupIcon, scaledSize: new window.google.maps.Size(46, 46) }} title="Pickup Location" />
          )}
          {destinationCoords && (
            <Marker position={destinationCoords} icon={{ url: dropOffIcon, scaledSize: new window.google.maps.Size(46, 46) }} title="Hospital Location" />
          )}
        </GoogleMap>
      </div>
      ...
      {showPaymentMethod && (
        <div className="payment-method">
          <div className="payment-method-content">
            ...
            <button className="book-cancel-btn" onClick={() => setShowPaymentMethod(false)}>Cancel</button>
          </div>
        </div>
      )}
    </div>
