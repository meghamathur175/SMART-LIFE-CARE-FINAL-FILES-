...
//     if (!phoneRegex.test(formData.hospitalPhone)) {
...
//       const hospitalPartnerId = decoded.hospitalPartnerId;

//       const res = await fetch("http://localhost:3001/api/partners-register/self-register", {
//         method: "POST",
//         headers: {
...
//           { label: "Email (Provided during Partner Onboarding)", name: "email", type: "email" },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        "http://localhost:3001/api/partners-register/self-register",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (res.status === 200 || res.status === 201) {
        alert("Registration request submitted. Awaiting admin verification.");
    ...
    <div className="auth-container">
      <form className="partner-form" onSubmit={handleSubmit}>
        ...
        {[
          ...
          { label: "Hospital Phone", name: "hospitalPhone", type: "text" },
          {
            label: "Email (Provided during onboarding)",
            name: "email",
            type: "email",
          },
          { label: "Password", name: "password", type: "password" },
