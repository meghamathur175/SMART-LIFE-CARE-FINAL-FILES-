...
const AgencyLoginForm = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/agencyregister/login", payload);
      const data = res.data;

      if (data.token && data.partner) {
        localStorage.setItem("agency", JSON.stringify(data.partner));
        localStorage.setItem("agencyToken", data.token);
        localStorage.setItem("agencyId", data.partner.id);
        localStorage.setItem("agencyName", data.partner.firstName);
  ...
  return (
    <div className="partner-login-box">
      <h2 className="form-title">Log In as Agency</h2>
      ...
      <div className="extra-links">
        <Link to="/agency-forgot-password" className="forgot-link">
          Forgot Password?
        </Link>
        <p>
          Don't have an account?{" "}
          <Link to="/agency-register" className="register-link">
            Register
          </Link>
        </p>
      </div>
